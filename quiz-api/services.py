...
def update_question(updated_question,question_id: int):
    ...
    # f"WHERE id = {question_id!r}"])
            # list_query.append(
            # f"UPDATE Question SET text = 'update position RUssie' "
            # f"WHERE id = {question_id!r}"
            # )
    ...
    # list_query.append(f"DELETE FROM Answer WHERE question_id = {question_json['id']!r}")
            ...
            list_query.append(
                ...
                f"{answer_query[:-1]}")
        # print(list_query)
            insert_answer,status_answer = update_statement(list_query)
            if not status_answer == 200 :
                return insert_answer,status_answer
            return insert_question,200
            # return 
    # return update_statement([f"UPDATE Question SET position = 100 "
    #     f"WHERE id = {question_json['id']!r}"])
